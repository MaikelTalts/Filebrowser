...
// == == == == == == == == == == BUTTON CLICK == == == == == == == == == == == //
$('.privileges-1').on('click', function(){                                        //Käyttäjän klikatessa update painiketta, saman rivin valinta, muutettavan ID sekä success ja error kuvakkeet lähetetään funktiolle sitä kutsuttaessa.
  user_privilege($(this).siblings('.select'), $(this).siblings('.userId'), $(this).siblings('.privileges_success'), $(this).siblings('.privileges_error'));
});

$('.select').on('click', function(){                                              //Käyttäjän klikatessa mitä tahansa select dropdownia kaikki tähänastiset success ja error kuvakkeet poistetaan näkyvistä
...
$('.delete-user').on('click', function(){                                         //Varmistetaan käyttäjän poisto
...
$('.btn-delete').on('click', function(e) {                                      //Haetaan käyttäjältä varmistus tiedoston poistoon.
  if(!confirm("haluatko varmasti poistaa tämän tiedoston?")) {
    e.preventDefault();
  }
});

$('.btn-delete-folder').on('click', function(e){                                //Haetaan käyttäjältä varmistus tiedostojen poistoon.
...
$('.rename').on('click', function() {
  ...
  showInputForRename($(this).siblings('.file_name_input'), $(this).siblings('.file_name'));
});
...
$('.confirm').on('click', function(){
  ...
  $('.rename').show(250);
  $('.download').show(250);
  switchBackToSpan($(this).siblings('.file_name'), $(this).siblings('.file_name_input'));
});
...
$('.cancel').on('click', function(){
  ...
  $('.rename').show(250);
  $('.download').show(250);
  returnOldName($(this).siblings('.file_name'), $(this).siblings('.file_name_input'));
});
...
function showInputForRename(input, span) {                                          //Tuodaan input näkyville, tiedoston aikasiemmalla nimellä
  Old_name = span.text();                                                           //Noudetaan vanha nimi
  adress = document.getElementById('adress').innerHTML;
  $(input).show();
  $(input).val(Old_name.replace(adress+"/", ""));
  $(span).hide();
}

function returnOldName(span, input) {                                               //Jos käyttäjä valitsee [Cancel], tuodaan vanha <span> teksti takaisin. (Vastaanotetaan span ja input elementit)
  $(input).hide();                                                                  //Piilotetaan input kenttä näkyvistä
  $(span).text(Old_name);                                                           //Muutetaan span elementin tekstiksi tiedoston vanha nimi
  $(span).show();                                                                   //Tuodaan span elementti näkyville
}

// == == == == == == == == == == AJAX  == == == == == == == == == == == //
function switchBackToSpan(span, $input) {                                           //Suoritetaan nimenmuutos palvelimella. Onnistuessaan muuttaa nimen palvelimella ja sivulla. Epäonnistuessa palauttaa sivulle aikaisemman nimen, palvelimella ei muutosta.
    $.ajax({                                                                        //Ajax pyyntö joka lähettää mukanaan tiedoston vanhan nimen, uuden nimen, sekä istunnon CSRF tokenin
      method: 'POST',
      url: urlRename,
      data: {OldName: Old_name, NewName: adress+"/"+$input.val(), _token: token},
      success: function(response){                                                  // Jos [FileController@rename palauttaa success]:in suoritetaan, nimen visuaalisella puolella nimenvaihto uuteen, ja näytetään preloade ennen nimen vaihtoa.
                  $( "#rename_notification_success" ).fadeIn( "fast");              //Tuodaan preloader alert elementti nopeasti näkyville.
                  $('#loader1').show("fast").delay(2000).hide("fast", function(){   //Tuodaan preloader näkyville, joka pyörii 2sec, jonka jälkeen katoaa.
      ...
      error: function(response){                                                    //Jos [FileController@rename palauttaa error] koodin, suoritetaan nimen palautus ennalleen
        returnOldName(span, $input);                                                //Kutsutaan returnOldName funktiota, lähetetään span ja input elementit mukana.
        $( "#rename_notification_danger" ).fadeIn( "slow");                         //Jos nimenmuutos ei onnistunut tulostetaan virheilmoitus
      }
    });
}

function user_privilege(selection, user, success, error){                           //Vastaanotetaan tarkistetaan valinta, käyttäjä, sekä onnistumisesta ja epäonnistumisesta kertovat elementit.
  ...
  $.ajax({                                                                          //Suoritetaan käyttöoikeuksien muutos ajax kutsuna
    ...
    success: function(response){                                                    //Jos muutos onnistuu tuodaan onnistumisesta kertova elementti näkyville.
    ...
    error: function(response){                                                      //Jos muutos epäonnistuu todaan epäonnistumisesta kertova elementti näkyville.
...
function delete_user(userId){                                                       //Käyttäjän poisto funktio ottaa vastaan käyttäjän elementin.
...
function createDirectory(value){                                     //Luodaan kansio ajax kutsulla.
...
$.ajax({
  ...
  success: function(response){
    ...
    console.log(path);
        $('.directories').append(
          $('<li></li>').attr('class', 'list-group-item folder').append(
            $('<a></a>').attr('href',"/" + path+"/"+value).append(
                $('<span>').attr('class', 'tab').append(
                  path+"/"+value))
      ).append($('<a>Delete</a>').attr({
...
for (i=0; i<count;i++){                                                             // Suoritetaan for loop (Niin monesti, kuin on kansiopolussa kansioita)
